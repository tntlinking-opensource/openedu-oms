...
public class FinanceAction extends ProcessLinkActionSupport {
	...
	protected OqlBuilder<?> getOqlBuilder() {
		...
		if (finished != null) {
			if (finished) {
				query.where("exists(from "+FinanceStudent.class.getName()+" f where student.id = f.student.id and f.feeOdd = 0)");
			} else {
				query.where("exists(from "+FinanceStudent.class.getName()+" f where student.id = f.student.id and f.feeOdd > 0)");
			}
		}
...
//		FinanceStudent financeStudent = financeService.getFinanceStatus(student);
		...
		FinanceGreenStd financeGreenStd = financeGreenCollectService.get(student.getUser().getId());
...
//		put("financeGreenStudents", financeGreenStudents);
	...
	public String handleBatch(){
		Long[] studentIds = getEntityIds();
		Long batchId = getLong("batchId");
		Long itemId = getLong("itemId");
		for(int i = 0; i < studentIds.length; i++){
			processLinkLogService.saveLog(batchId, itemId, studentIds[i]);
			welcomeService.processLinkPassed(batchId, itemId, studentIds[i]);
		}
		return redirect("search", "批量设置成功！");
	}
