...
[@b.form action="!sortSave" title="应用排序" theme="list"]
    ...
    <ol class="isortable isortable_all">
        [#list welcomeLinks?sort_by("sort") as b]
            <li data-group-id="${(b.group.id)!}">
                <img src="${base}/common/download.action?uuid=${(b.img)!}" style="height: 30px;"/>
                <span>${b.name!}</span>
                <input type="hidden" name="welcomeLink" value="${b_index}"/>
                <input type="hidden" name="welcomeLink${b_index}.id" value="${b.id}"/>
                <input type="hidden" class="sort_ipt" name="welcomeLink${b_index}.sort" value=""/>
