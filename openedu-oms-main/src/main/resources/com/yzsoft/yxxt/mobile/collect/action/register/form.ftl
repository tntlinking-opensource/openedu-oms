...
[#include "macro.ftl"/]
[@m.body title=type.name back=b.url("!info?id=${(type.id)!}&cardcode=${(student.cardcode)!}")]
<div class="yx-xxtx">
    <form action="${b.url("!save")}" class="studentInfoForm" method="post">
        ...
        <div class="yx-model-btnmodel">
            ...
            <input type="hidden" name="student.education.id" value="${(student.education.id)!}"/>
            <input type="hidden" name="cardcode" value="${(student.cardcode)!}"/>
